...
let signer;
const nftAbi=["constructor(string tokenName, string symbol)",
...
const auctionAbi=[
    "event Bid(address indexed bidder, uint256 listingId, uint256 amount, uint256 timestamp)",
...
async function list(){
    ...
    const minPrice=document.getElementById("min-price-list").vlaue;
    const duration=document.getElementById("list-duration").value;
    await auctionContract.list(nftAddress,id,minPrice,duration).then(()=>alert("success")).catch((error)=>{if(error.data) alert(error.data.message); else alert(error)})
...
async function end(){
    ...
    await auctionContract.end(id).then(()=>alert("success")).catch((error)=>{if(error.data) alert(error.data.message); else alert(error)})
}
...
async function approve(){
    await getAccess();
    const id=document.getElementById("listing-id-approve").value
