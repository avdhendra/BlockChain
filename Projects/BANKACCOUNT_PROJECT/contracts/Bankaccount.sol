...
contract Bankaccount {
    ...
    event WithdrawRequested(address indexed user,uint indexed accountId,uint withdrawId,uint amount,uint timestamp);
...
modifier validOwners(address[] calldata owners){
    require(owners.length+1<=4,"maximum of 4 owner per account");
...
modifier canApprove(uint accountId,uint withdrawId){
    ...
    require(accounts[accountId].withdrawRequests[withdrawId].user!=msg.sender,"you  cannot approved this request");
    require(accounts[accountId].withdrawRequests[withdrawId].user!=address(0),"this request does not exists");
    require(!accounts[accountId].withdrawRequests[withdrawId].ownersApproved[msg.sender],"you have already this request");
    _;
}
...
function deposit(uint accountId) external payable accountOwner(accountId){
    //modifier to check wether you are the owner of the account or not
 accounts[accountId].balance+=msg.value; //add the deposit value to balance of accountId
}
...
for(uint idx;idx<owners.length;idx++){
    if(idx<owners.length-1){
        owners[idx]=otherOwners[idx]; //copy the otherOwners to the Owners array
    }
...
function requestWithdrawl(uint accountId,uint256 amount) external accountOwner(accountId) sufficientBalance(accountId,amount){
